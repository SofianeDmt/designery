...
export default function PopupWidget() {
  ...
  const userName = useWatch({ control, name: "name", defaultValue: "Designery" });
  ...
  const onSubmit = async (data, e) => {
    ...
    await fetch("https://api.web3forms.com/submit", {
      ...
      .catch((error) => {
        setIsSuccess(false);
        setMessage("Erreur client réessayer dans quelques minutes");
        console.log(error);
      });
  };
  ...
  return (
    <div>
      <Disclosure>
        {({ open }) => (
          <>
            <Disclosure.Button className="fixed animate-bounce z-40 flex items-center justify-center transition duration-300 bg-indigo-500 rounded-full shadow-lg right-5 bottom-5 w-14 h-14 focus:outline-none hover:bg-indigo-600 focus:bg-indigo-600 ease">
            ...
            <Transition
              ...
              <Disclosure.Panel className=" flex flex-col  overflow-hidden left-0 h-full w-full sm:w-[350px] min-h-[250px] sm:h-[600px] border border-gray-300 dark:border-gray-800 bg-white shadow-2xl rounded-md sm:max-h-[calc(100vh-120px)]">
                <div className="flex flex-col items-center justify-center h-32 p-5 bg-indigo-600">
                  <h3 className="text-lg text-white">Comment puis-je vous aidez?</h3>
                  <p className="text-white opacity-50 text-sm">
                    Nous vous répondrons en quelques minutes
                  </p>
                </div>
                <div className="flex-grow h-full p-6 overflow-auto bg-gray-50 ">
                  {!isSubmitSuccessful && (
                    <form onSubmit={handleSubmit(onSubmit)} noValidate>
                      <input
                        type="hidden"
                        value="8f4ede7b-b587-4d4a-8eb8-a7e1c3702059"
                        {...register("apikey")}
                      />
                      <input
                        type="hidden"
                        value={`${userName} a envoyer un message depuis Designery`}
                        {...register("subject")}
                      />
                      <input
                        type="hidden"
